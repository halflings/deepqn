...
def _generate_sample(state_dim, n_actions):
    return np.random.rand(state_dim), np.random.randint(0, n_actions), np.random.rand(), np.random.randint(0, 2)
...
def test_memory():
    ...
    for i in range(items_to_add):
        state, action, reward, terminal = _generate_sample(state_dim, n_actions)
    ...
    prestates, actions, states, rewards, terminal = memory.sample(memory_size // 2)
    ...
    prestates1, states1, actions1, rewards1, terminals1 = memory.sample(memory_size - 1)
    prestates2, states2, actions2, rewards2, terminals2 = memory.sample(memory_size - 1)
    assert not (prestates1 == prestates2).all()
    assert not (states1 == states2).all()
    assert not (actions1 == actions2).all()
    assert not (terminals1 == terminals2).all()
